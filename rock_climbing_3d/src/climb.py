...
class HoldGeometry:
    ...
    def best(self, x: float, y: float, margin: float, allow=None):
        """The hold this point is most inside, or None. Ties broken by depth.

        *allow* is an optional ``hold -> bool``: a hold it refuses is not a
        candidate at all, so the next-best one can win instead.
        """
        best_hold, best_depth = None, -margin
        for hold in self.holds:
            d = self.depth(hold, x, y)
            if d >= -margin and d > best_depth:
                # Asked only of holds the image already accepts, so a refusal
                # is always a contact the 2D test alone would have made.
                if allow is not None and not allow(hold):
                    continue
...
def limb_point(poses, frame: int, limb: Limb, *, cfg, toe_offset: float | None = None):
    ...
    *toe_offset* is how far below the ankle joint to put the contact point, in
    image units — :func:`scales` derives it from the climber's own height, so
    it is a foot rather than whatever fraction of the frame the camera happened
...
def scales(holds: list[dict], poses) -> tuple[float, float]:
    ...
    A fraction of the frame is only a fixed distance on the rock when the
    boulder fills the frame the same way every time, and a handheld camera
    does not: it zooms in, pulls back, and frames ceiling and mats. So the
    thresholds are not fractions of anything the camera decides, but multiples
    of two things the scene decides:
...
def analyze(holds: list[dict], poses, *, fps: float, aspect: float, cfg,
            floor=None, per_frame=None) -> ClimbAnalysis:
    """Walk the clip once, in order, and record what each limb held.

    *per_frame* switches the geometry from fixed to projected, which is how a
    climb is read in 3D: each hold is projected into each image and the
    keypoint is compared to it there. That is the exact test rather than
    a stand-in for it: a hand at pixel *p* is on hold *h* precisely when *p*
    falls inside *h*'s projection, and no intermediate surface has to be
    invented to decide it.

    Pass a `space.FrameGeometry`; it answers `.at(frame)` with that frame's
    geometry and its own hold scale, so the margins keep meaning one distance on
    the rock as the camera moves toward and away from it.
    """
    frames = poses.frames()
    ...
    # A 3D gate, when the representation has depth to offer one: see
    # `lidar.ContactGate`. It only ever removes candidates, so with no gate or
    # no depth this is the 2D tracker exactly.
    gate = getattr(per_frame, "gate", None) if per_frame is not None else None

    for frame in frames:
        if per_frame is not None:
            geometry, hold_unit = per_frame.at(frame)
            enter = cfg.HOLD_MASK_MARGIN * hold_unit
            release = cfg.HOLD_RELEASE_MARGIN * hold_unit
            graze_depth = cfg.HOLD_GRAZE_DEPTH * hold_unit
        ...
        for limb in LIMBS:
            ...
            # is the answer.
            if gate is not None:
                def allow(hold, keeping=False, _limb=limb, _frame=frame):
                    return gate.allows(_frame, _limb.kpt, hold["id"], keeping=keeping)
            else:
                allow = None
            current = geometry.by_id.get(on[key]) if on[key] is not None else None
            if current is not None and allow is not None and not allow(current, keeping=True):
                current = None      # the depth says the limb has left it
            depth = geometry.depth(current, *point) if current is not None else None
            if depth is not None and depth >= -release:
                better = geometry.best(*point, margin=enter, allow=allow)
                ...
                chosen = geometry.best(*point, margin=enter, allow=allow)
                depth = None
        ...
        if start_frame is None:
            if floor is not None:
                ...
                for limb in LIMBS:
                    ...
                    # are then a couple of centimeters apart and the floor test
