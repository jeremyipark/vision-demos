...
def pick_track(items: list[dict], *, mask, overlap_fn, min_overlap: float,
               lock_to_wall: bool, per_frame: bool = False
               ) -> tuple[int | None, dict]:
    ...
    # *per_frame* says `overlap_fn` wants the frame number, and will project the
    # route into that frame itself; without it, *mask* is a fixed image the boxes
    # are scored against.
    for track, records in by_track.items():
        if per_frame:
            overlaps = [overlap_fn(r["bbox_xywh"], r.get("frame_id", 0))
                        for r in records]
    ...
    if lock_to_wall and (mask is not None or per_frame):
